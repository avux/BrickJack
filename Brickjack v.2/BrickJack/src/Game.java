...
//import java.time.Instant;
	...
	public class Game {
		...
		CardDealer dealer = new CardDealer(scanner);
		...
		public void moderate() {		    
		    ...
		    while(stillPlaying > 0) {
		        ...
		        if (!finished[playerNum]) { // Dealer takes card or not
		        	sums[playerNum] += this.giveCard(playerNum, false);
		    ...
		    winners(); // displaying win messages
		    bigBrain.writeInt(-1); // telling the server to stop listening
		    bigBrain.writeInt(-1);
		    Delay.msDelay(10000); // waiting so that the win message can be read
		}
		...
		public int giveCard(int num, boolean cheating) {
		    if (num == playerNum) {
		    ...
		    int card;
		    if (cheating) { // this would be cheating. Executed if flag cheating is set.
		    	card = cheatdraw();
		    } else { // this is the noble way to do things.
		    	dealer.dealCard();
				int[][] data = scanner.move();
				bigBrain.sendIntData(data);
			    card = bigBrain.readInt(); 
		    }
